...
export function classifyIP(ip) {
  if (!net.isIP(ip)) return { kind: 'invalid', label: 'invalid' };
  if (net.isIPv6(ip)) {
...
export function parseTarget(input) {
  ...
  if (net.isIP(candidate)) {
    return { type: net.isIPv6(candidate) ? 'ipv6' : 'ipv4', value: candidate, asciiHost: candidate };
  }
...
// Resolve host to first PUBLIC IP. If any answer is blocked → throw BLOCKED_TARGET.
async function resolvePublic(hostname) {
  const ips = [];
  try { (await resolve4(hostname)).forEach(ip => ips.push({ ip, family: 4 })); } catch { /* no A */ }
  try { (await resolve6(hostname)).forEach(ip => ips.push({ ip, family: 6 })); } catch { /* no AAAA */ }
